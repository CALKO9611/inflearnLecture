...
def add_file():
    files = filedialog.askopenfilenames(
        ...
        initialdir="C:/Users/SeoWon/Desktop/Coding/inflearnLecture",
...
def merge_image():
    ...
    if img_width > -1:
        # width 값 변경
        image_sizes = [
            (int(img_width), int(img_width * x.size[1] / x.size[0])) for x in images
        ]
    else:
        # 원본 사이즈 사용
    ...
    dest_path = os.path.join(txt_dest_path.get(), "calko_photo.jpg")
...
txt_dest_path.pack(
    side="left", fill="x", expand=True, padx=5, pady=5, ipady=4
)  # 높이 변경
